...
class Square:
    def __init__(self, x, y, color, idd, chunk_ids, value, scale, obraz):
        self.x = x
        self.y = y
        self.value = value
        self.color = color
    ...
    def update_position(self):
        self.position = [(self.x * self.scale) + (self.chunk_ids[0] * self.scale * 8) + self.ofpo[0], (self.y * self.scale) + (self.chunk_ids[1] * self.scale * 8) + self.ofpo[1]]
    ...
    def draw_shade(self, Ekran, multer):
        if self.visible_on_the_scrreen:
            ...
            if self.have_shade:
                if (self.x * self.scale) + (self.chunk_ids[0] * self.scale * 8) + self.ofpo[0] < Ekran[0] / 2:
                    self.part_of_screen[0] = 0
                else:
                    self.part_of_screen[0] = 1
                if (self.y * self.scale) + (self.chunk_ids[1] * self.scale * 8) + self.ofpo[1] < Ekran[1] / 2:
                ...
                self.Dx = (self.x * self.Mult) + (self.chunk_ids[0] * self.Mult * 8) + (self.ofpo[0] + (Ekran[0] * multer))/(1 + (2 * multer))
                self.Dy = (self.y * self.Mult) + (self.chunk_ids[1] * self.Mult * 8) + (self.ofpo[1] + (Ekran[1] * multer))/(1 + (2 * multer))
    ...
    def draw_square_border(self):
        pygame.draw.rect(self.obraz, [0, 0, 0], [(self.x * self.scale) + (self.chunk_ids[0] * self.scale*8) + self.ofpo[0], (self.y * self.scale) + (self.chunk_ids[1] * self.scale*8) + self.ofpo[1], self.scale, self.scale], 2)
    ...
    def create_small_squares(self, obraz):
        for x in range(4):
            for y in range(4):
                    ...
                    if a > 1000:
                        self.small_squares[(x, y)] = SmallSquare(x, y, [0, 40 + a-1000, 0], self.idd, (x, y), self.chunk_ids, self.value, self.scale, self.obraz)
